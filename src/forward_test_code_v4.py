...
# ==============================================================================
# MAIN ENGINE WITH CORRECTED CHRONOLOGICAL LEDGER SUMMARY
# ==============================================================================
def check_live_signals():
        ...
        if market_ok and available_slots > 0:
            ...
            for t in TICKERS:
                ...
                if sys['name'] == 'MODEL A (YANG)':
                    ...
                    if not (atr50 > 0 and atr14 > 2 * atr50) and close_spot > df['Close'].rolling(200).mean().iloc[-1]:
                        if rsi2 < 5 or rsi2 < 10:
                            trigger_entry, mod_type = True, f"Yin Module {'B' if rsi2 < 5 else 'A'}"
            ...
            for c in candidates:
                ...
                if risk_dist > 0:
                    ...
                    if final_shares > 0:
                        print(f"    ⭐ [ENTRY] Ticker: {c['ticker']} | Volume: {final_shares} Shares | Entry Price: ${friction_entry:.2f}")
                        sys['positions'][c['ticker']] = {'sector': SECTORS[c['ticker']], 'shares': final_shares}
                        # Deduct capital dynamically from cash to model real post-execution states
    ...
    # ==============================================================================
    # 📋 EXCEL ONE-CLICK COPY DASHBOARD
    # ==============================================================================
    print("==============================================================================")
    print("          📋 EXCEL ONE-CLICK COPY DASHBOARD (TAB-DELIMITED FORMAT)")
    print("==============================================================================")
    print(f"👉 FOR TAB 1 (Global Market Filter) -> Paste into Cell A{TARGET_EXCEL_ROW}:\n{tab1_log}\n")
    print(f"👉 FOR TAB 2 (Yang Portfolio Row)  -> Paste into Cell A{TARGET_EXCEL_ROW}:\n{excel_outputs['YANG']}\n")
    print(f"👉 FOR TAB 3 (Zhan Portfolio Row)  -> Paste into Cell A{TARGET_EXCEL_ROW}:\n{excel_outputs['ZHAN']}\n")
    print(f"👉 FOR TAB 4 (Yin Portfolio Row)   -> Paste into Cell A{TARGET_EXCEL_ROW}:\n{excel_outputs['YIN']}\n")
    print("==============================================================================")
