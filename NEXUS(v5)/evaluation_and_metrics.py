...
# ==========================================
# BENCHMARK - SPX BUY & HOLD (Matrix Protected)
# ==========================================
def run_benchmark(sp500, dates, ctx: SystemContext):
    ...
    # FIXED: Extract scalar index value even if input carries Multi-Index layouts
    raw_sp500 = sp500.squeeze() if isinstance(sp500, pd.DataFrame) else sp500
    base_price = float(raw_sp500.loc[valid_dates[0]])
    ...
    for i in range(1, len(valid_dates)):
        ...
        # FIXED: Extract clean float elements to prevent column tracking leakages
        current_price = float(raw_sp500.loc[date])
        equity = ctx.START_CAPITAL * (current_price / base_price)
...
def compare_models(results, ctx: SystemContext, SHOW_CHARTS=False):
    ...
    # FIXED: Flexible search mapping ensures alignment even if string names have run variations
    bench_row = table[table['Model'].str.contains('spx|benchmark|hold', case=False, na=False)]
    if not bench_row.empty:
        bench_name = bench_row['Model'].values[0]
        ...
        print("==============================================================")
        for m in metrics:
            if m['name'] == bench_name:
                continue
            excess_return = m['total_return'] - bench_return
            excess_sharpe = m['sharpe'] - bench_sharpe
            print(f"{m['name']:<25}: {excess_return:+7.2f}% return vs SPX | {excess_sharpe:+.2f} Sharpe vs SPX")
    ...
    for m in metrics:
        ...
        # Clean export name string generation
        clean_name = m['name'].lower().replace(' ', '_').replace('(', '').replace(')', '')
        csv_name = f"{clean_name}_trades.csv"
