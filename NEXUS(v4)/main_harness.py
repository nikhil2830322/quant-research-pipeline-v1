...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------------------------
    # UNLOCKED: COMPREHENSIVE PIT MATRIX INGESTION
    # Strips out the clean_core_tickers restriction completely.
    # Ingests every single historical constituent from the CSV rows.
    # --------------------------------------------------------------------------
    ...
    for date_str, ticker_list in raw_universe_history.items():
        ...
        for t in ticker_list:
            # Sanitize tickers to prevent string layout mismatches
    ...
    # Phase 1: Ingest Data Elements safely for ALL historical index candidates
    ...
    # OLD CODE: max_total_positions=len(all_historical_tickers)
    # NEW FAST CODE:
    yang_free, zhan_free, yin_free = run_full_suite(
        ind, data, sp500, sp500_sma200, sp500_sma50,
        max_total_positions=30, max_alloc_fraction=1.0, max_sector_positions=30,
        label="(Unconstrained)",
        base_ctx=master_ctx
    )
    ...
    try:
        ensemble_results = run_ensemble_suite(
            ...
            universe_history=sanitized_universe_history  # Pass fully unlocked map
        )
    except TypeError:
